...
class Home extends CI_Controller {
	...
	public function index($mode='')
	{
            $data = array(
                'BASE_URL'      => base_url(),
                'SITE_URL'      => site_url(),
                'BODY_TITLE'    => 'Informasi',
				'FIELDS'		=> $this->info_mdl->get_fields_list(),
            );
            ...
            $data['SCRIPT_SECTION']     = $this->parser->parse('js/script_info', $data, true);            	
    ...
    public function form($mode='', $id=0)
	{
            $data = array(
                ...
                'SITE_URL'      	=> site_url(),
                'BODY_TITLE'    	=> 'Informasi',
				'FORM_FIELDS'		=> $this->info_mdl->get_form_fields_list(),
            );
            $data['HEADER_SECTION']     = $this->parser->parse($this->themes.'/layout/header/header', $data, true);
            $data['BODY_SECTION']       = $this->parser->parse($this->themes.'/layout/menu/sidebar', $data, true);
