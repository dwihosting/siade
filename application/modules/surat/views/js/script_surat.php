            ...
            $('#surat_tables').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,                
                "sAjaxSource": "{SITE_URL}surat/home/list_table/{title}",
                 "sServerMethod": "POST",
                ...
                "aoColumnDefs": [
                    ...
                    {'sWidth': '10%', 'bSortable': false,'aTargets': [2]},
                    {'sWidth': '15%', 'bSortable': false,'aTargets': [3]},  
                    {'sWidth': '20%', 'bSortable': false,'aTargets': [4]},  
                    {'sWidth': '25%', 'bSortable': false,'aTargets': [5]},
                    {'sWidth': '15%', 'bSortable': false,'aTargets': [6]}  
                ]
            });

            jQuery('#surat_tables .dataTables_filter input').addClass("form-control input-medium"); // modify table search input
            jQuery('#surat_tables .dataTables_length select').addClass("form-control input-small"); // modify table per page dropdown
            ...
            $('body').on('click', '#surat_tables .btn-editable', function() {
                window.location.href="{LINK_ADD_NEW}/" + $(this).attr("data-id");
            });

            $('body').on('click', '#surat_tables .btn-removable', function() {
                var id = $(this).attr("data-id");
                $.ajax({
                    type: "POST",
                    url: "{SITE_URL}surat/home/hapus",
                    data: {id:id},
                    success: function() {
