            ...
            $('#pekerjaan_tables').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,                
                "sAjaxSource": "{SITE_URL}master/pekerjaan/list_table",
                 "sServerMethod": "POST",
            ...
            jQuery('#pekerjaan_tables .dataTables_filter input').addClass("form-control input-medium"); // modify table search input
            jQuery('#pekerjaan_tables .dataTables_length select').addClass("form-control input-small"); // modify table per page dropdown
            ...
            $('body').on('click', '#pekerjaan_tables .btn-editable', function() {
                window.location.href="{LINK_ADD_NEW}/" + $(this).attr("data-id");
            });

            $('body').on('click', '#pekerjaan_tables .btn-removable', function() {
                var id = $(this).attr("data-id");
                $.ajax({
                    type: "POST",
                    url: "{SITE_URL}master/pekerjaan/hapus",
                    data: {id:id},
                    success: function() {
