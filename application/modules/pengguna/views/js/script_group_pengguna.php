            ...
            // begin first table
            $('#group_pengguna_tables').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,                
                "sAjaxSource": "{SITE_URL}pengguna/group_akun/list_table",
                 "sServerMethod": "POST",
                ...
                "aoColumnDefs": [
                    ...
                    {'sWidth': '75%', 'bSortable': false,'aTargets': [1]},
                    {'sWidth': '15%', 'bSortable': false,'aTargets': [2]},                    
                ]
            });

            jQuery('#group_pengguna_tables .dataTables_filter input').addClass("form-control input-medium"); // modify table search input
            jQuery('#group_pengguna_tables .dataTables_length select').addClass("form-control input-small"); // modify table per page dropdown
            ...
            $('body').on('click', '#group_pengguna_tables .btn-editable', function() {
                alert('Edit record with id:' + $(this).attr("data-id"));
                window.location.href="{LINK_ADD_NEW}/" + $(this).attr("data-id");;
            });

            $('body').on('click', '#group_pengguna_tables .btn-removable', function() {
